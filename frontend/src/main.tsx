...
function Landing() {
  return <div className="min-h-screen bg-slate-950 text-slate-100"><div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20"><p className="text-sm text-sky-300">Azure Blob Training Project</p><h1 className="mt-3 text-3xl font-bold sm:text-5xl">Insurance Claims Platform</h1><p className="mt-4 max-w-3xl text-sm text-slate-300 sm:text-base">Secure upload, claim review, private document viewing, and auditable status flow.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link to="/login"><Button className="w-full sm:w-auto"><Shield size={16}/>Login</Button></Link><Link to="/signup"><Button className="w-full sm:w-auto" variant="ghost">Create Account</Button></Link></div><div className="mt-10 grid gap-4 md:grid-cols-3"><Card><CardHeader><CardTitle>Blob Security</CardTitle></CardHeader><CardContent>Private container, scoped read access, and tracked document metadata.</CardContent></Card><Card><CardHeader><CardTitle>Claims Workflow</CardTitle></CardHeader><CardContent>User submission with admin approval/rejection and comments.</CardContent></Card><Card><CardHeader><CardTitle>Operational Visibility</CardTitle></CardHeader><CardContent>Dashboards for stats, users, and full claim lifecycle.</CardContent></Card></div></div></div>;
}
...
function AppShell(){
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100"><nav className="sticky top-0 z-10 border-b border-slate-800 bg-slate-950/90 backdrop-blur"><div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-6"><h2 className="font-semibold">Insurance App</h2><div className="flex flex-wrap gap-2">{links.map((l)=><Link key={l.to} to={l.to} className="rounded px-3 py-2 text-sm text-slate-300 hover:bg-slate-800">{l.label}</Link>)}</div></div><Button size="sm" variant="ghost" onClick={()=>{localStorage.removeItem("token"); location.href="/";}}><LogOut size={14}/>Sign out</Button></div></nav><main className="mx-auto max-w-6xl space-y-4 px-3 py-4 sm:px-4 sm:py-6"><Routes><Route path="/" element={user.role==="ADMIN"?<AdminDashboard/>:<UserDashboard/>}/><Route path="/new-claim" element={<ClaimCreate/>}/><Route path="/claims/:id" element={<ClaimDetails/>}/><Route path="/admin/claims" element={<AdminClaims/>}/><Route path="/admin/users" element={<AdminUsers/>}/></Routes></main></div>;
}
